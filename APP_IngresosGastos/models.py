...
def select_all():
    connection = ConnectDatabase("SELECT id, Date, Description, Value FROM movements order by date;", None)
...
def select_by(id):
    connection = ConnectDatabase(f"SELECT id, Date, Description, Value FROM movements WHERE id={id}", None)
    record = connection.res.fetchall()
    connection.con.close()
    return  record[0] 

def delete_by(id):
    connection = ConnectDatabase(f"DELETE FROM movements WHERE id={id}", None)
    connection.con.commit()
    connection.con.close()